...
// TODO: update shape
const Slider: React.FC<{ items: any[] }> = ({ items }) => {
  const [sliderHasMoved, setSliderHasMoved] = React.useState(false); // boolean to display prev arrow
  const [sliderMoving, setSliderMoving] = React.useState(false); // boolean for slider animation
  const [movePercentage, setMovePercentage] = React.useState(0); // move percentage to shift slider during animation
  const [sliderMoveDirection, setSliderMoveDirection] = React.useState<
    string | null
  >(null); // direction of movement of animation
  const [lowestVisibleIndex, setLowestVisibleIndex] = React.useState(0); // lowest visible index of slider content
  const [itemsInRow, setItemsInRow] = React.useState(5); // number of items in the slider content changed dynamically on window size
